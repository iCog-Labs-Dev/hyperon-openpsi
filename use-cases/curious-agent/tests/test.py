...
class TestAdapter(unittest.TestCase):
    ...
    def test_parse_state_params(self):
        ...
        expected_state_params = StateParams(
            activation=0.5,
            securing_threshold=0.7,
            pleasure=0.8,
    ...
    def test_extract_rules_from_llm(self):
        ...
        expected_rules_single = [
            "(: R1 (IMPLICATION_LINK (AND_LINK (((Self at Door) (Self Has Key))) (Open Door)) ) (TTV 100 (STV 0.9 0.8))))"
        ]
        self.assertEqual(
            extract_rules_from_llm(raw_rules_single), expected_rules_single
        )
        ...
        expected_rules_whitespace = [
            "(: R1 (IMPLICATION_LINK (AND_LINK (((Self at Door) (Self Has Key))) (Open Door)) ) (TTV 100 (STV 0.9 0.8))))"
        ]
        self.assertEqual(
            extract_rules_from_llm(raw_rules_whitespace), expected_rules_whitespace
        )
        ...
        self.assertEqual(
            extract_rules_from_llm(raw_rules_extra_whitespace),
            expected_rules_extra_whitespace,
        )
        ...
        expected_rules_empty_rule = [
            "",
            "(: R1 (IMPLICATION_LINK (AND_LINK (((Self at Door) (Self Has Key))) (Open Door)) ) (TTV 100 (STV 0.9 0.8))))",
        ]
        self.assertEqual(
            extract_rules_from_llm(raw_rules_empty_rule), expected_rules_empty_rule
        )

    def test_parse_to_dict(self):
        output = parse_to_dict("(a 1 b 2 c 3)")
